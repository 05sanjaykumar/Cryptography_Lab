...
public class DESSender {
    ...
    private static Map<String, String> encryptByte(String plain64, List<String> subkeys) {
        Map<String, String> result = new LinkedHashMap<>();
        String ip = permute(plain64, IP);
        result.put("IP", ip);
        ...
        for (int round = 0; round < 16; round++) {
            String[] temp = fk(L, R, subkeys.get(round));
            L = temp[0];
            R = temp[1];
            result.put("Round" + (round + 1), L + R);
        }
    ...
    public static void main(String[] args) {
        try (ServerSocket serverSocket = new ServerSocket(PORT);
            ...
            System.out.print("Enter the Message (text or 64-bit binary stream): ");
            ...
            boolean isBinary = plain.chars().allMatch(c -> c == '0' || c == '1') && plain.length() % 64 == 0;
            
            if (isBinary) {
                for (int i = 0; i < plain.length(); i += 64) {
                    String block = plain.substring(i, Math.min(i + 64, plain.length()));
                    blocks.add(block);
                    labels.add(block);
                }
            } else {
                for (int i = 0; i < plain.length(); i += 8) {
                    String chunk = plain.substring(i, Math.min(i + 8, plain.length()));
                    StringBuilder chunkBits = new StringBuilder();
                    for (char ch : chunk.toCharArray()) {
                        chunkBits.append(String.format("%8s", Integer.toBinaryString(ch)).replace(' ', '0'));
                    }
                    while (chunkBits.length() < 64) {
                        chunkBits.append('0');
                    }
            ...
            System.out.println("Final Cipher Text (bits): " + cipherText);
